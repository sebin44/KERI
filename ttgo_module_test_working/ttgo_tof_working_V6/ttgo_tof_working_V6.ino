  ...
  if (Serial2.available()>=32) {
    ...
    for(int j=0;j<16;j++)
    {
      if( (TOF_data[j]==TOF_header[0] && TOF_data[j+1]==TOF_header[1] && TOF_data[j+2]==TOF_header[2]) && (verifyCheckSum(&TOF_data[j],TOF_length)))
      {
        ...
        {
           SerialMon.println("Out of range!");
         }else{
           SerialMon.print("TOF id is: ");
           SerialMon.println(TOF_data[j+3],DEC);
    
           TOF_system_time = TOF_data[j+4] | TOF_data[j+5]<<8 | TOF_data[j+6]<<16 | TOF_data[j+7]<<24;
           SerialMon.print("TOF system time is: ");
           SerialMon.print(TOF_system_time,DEC);
           SerialMon.println("ms");
    
           TOF_distance = (TOF_data[j+8]) | (TOF_data[j+9]<<8) | (TOF_data[j+10]<<16);
           SerialMon.print("TOF distance is: ");
           SerialMon.print(TOF_distance,DEC);
           SerialMon.println("mm");
    
           TOF_status = TOF_data[j+11];
           SerialMon.print("TOF status is: ");
           SerialMon.println(TOF_status ,DEC);
    
           TOF_signal = TOF_data[j+12] | TOF_data[j+13]<<8;
           SerialMon.print("TOF signal is: ");
           SerialMon.println(TOF_signal ,DEC);
